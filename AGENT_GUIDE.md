...
| `<dir>/.kpot-runs/папки-на-согласование.txt` | The owner's decisions file. Folders with an unclear NAME are held, not sorted, until the owner writes `сортировать` or `как есть` against each. Regenerated every plan; existing answers are preserved. `--no-cache` does not affect it. |
