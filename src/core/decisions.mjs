...
export async function saveDecisions(root, suspicious, existing = new Map()) {
  ...
  const L = [
    ...
    '# программа. Поэтому KPOT не разбирает их, а откладывает целиком в папку НА_РАЗБОР —',
    '# как есть, вместе с их исходной структурой папок. Ничего внутри не тронуто.',
    '#',
    '# Напишите после знака = одно из двух:',
    `#     ${DECISION_SORT}  — разобрать файлы внутри по годам и порам года, как всё остальное`,
    `#     ${DECISION_AS_IS}     — так и оставить папку в НА_РАЗБОР, целиком, не разбирая`,
    '#',
