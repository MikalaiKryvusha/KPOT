...
- **Steps:** duplicate grouping across directories + keeper selection · season mapping (⚠️ blocked on
  the owner interview — `GOAL.md` names four buckets and omits autumn) · per-year and global "прочее"
  buckets · collision handling that preserves original names · the `plan` phase emitting the SortPlan,
  the disputed cases and the collisions.
...
- **Status:** 🔲 todo. Blocked on: season-boundary interview.
