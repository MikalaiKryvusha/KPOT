...
| 2026-07-29 | **KPOT LEAVES A RECEIPT, and stops deducing its own past from the shape of a folder.** Owner, verbatim: «KPOT должен оставлять документ-расписку. Его нет — считаем, что беспорядок. Он есть — видим в нём историю сортировок.» `src/core/receipt.mjs` writes a plain readable document into the archive root («KPOT — что здесь сделано.txt») listing the sorts that are **still in effect** — when, how many files, and the command to undo each. A real run that moved something records itself; an undone run is removed; when the last entry goes, so does the file. `libraryShape()` asks the receipt, so the wizard-or-panel decision rests on what KPOT DID, not on what the folder looks like. The document is excluded from the scan, parsed by run id rather than by prose, and written best-effort after the files are already home | This closes `bugs/06_DONE`, and it is a better answer than any of the four the agent had drafted — all of which tried to infer the past from the present and differed only in cleverness. The defect was real and landed on the owner's own archive: any top-level `20xx/` made the interface announce «Всё уже разложено» over an untouched heap, with «Пока ничего не запускалось» two blocks below on the same screen, and `researches/02` records that his 551 GB archive already holds hand-made `<year>/<season>` folders — so his first run would never have seen the wizard built for exactly his situation. A receipt does not infer. It also fails safe by construction (no document, no claim), it is something the person can open, read and delete — the product's own transparency applied to its own state — and it makes an undone sort stop counting, which closes the same bug by its other door |
...
| 2026-07-29 | **A BLANKET AUTHORISATION GRANTS EXECUTION, NEVER AUTHORSHIP OF IDENTITY.** After `bugs/07_DONE`: the owner pre-authorised release 0.2 («НА ВСЁ ДАЮ ДОБРО, МЕНЯ МОЖЕШЬ НЕ СПРАШИВАТЬ»), and the agent read that as covering the release CODENAME too — publishing a public name the owner first saw as a fait accompli («Было принято бренд решение без владельца»). The rule now in `AGENT_GUIDE.md`: a wide yes removes the confirmation friction on an ACTION; it never transfers authorship of what the product is CALLED. Naming — release codenames, product and feature names, taglines, any brand-visible string — stays the owner's under any width of approval. Under a blanket yes the correct move is to do everything else and ask the ONE naming question; where even that is impossible, ship with NO codename (a neutral factual title), never a placeholder. `/release` Step 0 is now a hard stop with exactly three legal outcomes. The owner named 0.2 himself: **«Obvius»** | The rule already existed («never make brand decisions without confirmation», with brand FIRST in the list) and the skill already said «ask the human for it» — both were read in-session and did not fire, and the agent recalled the rule correctly one message too late. So the missing thing was not a rule but a DISTINCTION between authorising an act and authorising a decision, plus the fact that the codename had no artifact of its own: every other release field (version, artifacts, hashes, test counts) is checked because each has a place where it is checked, while the name was free text typed into a title. A field with no recorded author is a field nobody checks |
| 2026-08-01 | **KAIF updated 1.6 → 2.1 «Strong KAIF», on the owner's direct order** — «тогда делай обновление KAIF до версии 2.1 из origin. потом контур вычитки по опыту NDim проекта» (chat, 2026-08-01). This is the one exception to his standing «я сам веду обновления КАИф» rule, and it covers this operation only — the rule itself stands | Route: bootstrap with the fresh thin `KAIF.md` v2.1 (the new core does the classification), predicted on a sandbox copy first; the live pass matched it exactly. 18 replaced · 24 modules merged · 13 added · 31 kept; owner documents and product code untouched. Migration record: `KAIF_FRAMEWORK.md`. What it brings that this project will actually use: the **`/owner-reviews` review contour** (the next task), the place-of-questions hard rule, the planning ladder, `PROJECT_HISTORY.md` (STATUS went 1204 → ~460 lines) |
| 2026-07-29 | **Release notes are the DELTA and link to the README; they never duplicate it** (`bugs/08`). Notes answer «what changed in this version, should I update?» and are written from this version's delta plus a brief quick start; the README answers «what is this and how do I use it?». The sources of inspiration differ, and that is the operative half: a README is written from the project's current README **and the neighbouring READMEs in the owner's other repositories** (his shopfront keeps his handwriting), while release notes are written from **this project's previous releases** (`gh release view <prev>`). Mechanical check before publishing: *if a paragraph could be pasted into the README unchanged, it belongs in the README* | 0.2 shipped with 34 KB of notes that were nearly a copy of the README — owner: «зачем в релиз ноутсы попала почти вся копия README - это НЕПРАВИЛЬНО!»; rewritten to 12 KB of pure delta, and he confirmed it («сейчас стало хорошо… супер»), including that the header image stays. The cause was the SKILL TEMPLATE, which asked for «a short *what X is* paragraph» and «a Get started section» — in a project whose README has both, that is an instruction to duplicate. Worth recording as its own class: a rule violated is loud, a rule OBEYED into the wrong outcome is silent, so the wording is fixed at the source rather than left to the agent's taste |
