...
| Поле | Значение |
|------|----------|
| **Версия KAIF** | `2.1` «Strong KAIF» (выпуск 2026-07-31) |
| **Инъекция выполнена** | `2026-07-24` (на версии `1.5`) |
| **Обновление 1.6 → 2.1** | `2026-08-01`, по прямому слову владельца («тогда делай обновление KAIF до версии 2.1 из origin»). Маршрут — **bootstrap** свежим тонким `KAIF.md` v2.1: загрузчик забрал машинерию 2.1 из origin со сверкой sha256, и классификацию выполняло уже НОВОЕ ядро. Старую команду `kaif-core update` не использовали намеренно: машинерия 1.6 разбирала бы бандл 2.1 пофайловой логикой, без модульного слияния и без синтетической базы, которая нужна нашему манифесту v1. Перед боевым проходом сделано **предсказание на песочной копии** (`git archive HEAD` → zip → `git init` → настоящий bootstrap в копии) — бой совпал с песочницей: те же счётчики, тот же набор файлов. Итог: **18 заменено · 24 модуля слито на месте · 13 добавлено · 31 сохранено · 6 adopted**; документы владельца и продуктовый код — 0 изменений; скиллов 26 → 34. Вручную сведено 6 файлов (`AGENT_GUIDE.md`, `/pause`+`/end-chat`, `/release`, три цикла) — в трёх местах шаблон стёр бы наш канон, и песочница предупредила об этом заранее. Летопись STATUS (1204 строки) переехала в `PROJECT_HISTORY.md` дословно, со сверкой по кириллице. Что принесла 2.1: `PROJECT_HISTORY.md`, лестница планирования (`/plan-task`/`/plan-epic`), `/guarded-loop`, `/code-revision`, `/owner-voice`, **`/owner-reviews`** (контур вычитки), `/end-chat` и мягкий `/pause`, `/derive-styleguide`, правило «место вопросов», класс вкуса, режимы строгости, механизированный провенанс-гейт |
| **Обновление 1.5 → 1.6** | `2026-07-26`. Механический проход (`node .kaif/kaif-core.mjs update`): 38 файлов заменено, 0 добавлено, 14 сохранено; машинерия `KAIF-CORE.mjs` побайтово совпадает с 1.5 (менялся только бандл шаблонов), обе загрузки сверены по sha256 с манифестом релиза. Вручную смержено 6 разошедшихся файлов (`AGENT_GUIDE.md`, `/autoloop`, `/dayloop`, `/nightloop`, `/release`, `.kaif/spheres/programming.md`) плюс `EXPERIENCE.md` (владельческий документ — машинерия его не трогает). Что принесла 1.6: разведартефакты (recon-док / канон-карта / инвентарь паритета), правило одного шага и git-гигиена, судья перед каждым push, деплой-чеклист из пяти гейтов, пометки провенанса `[AI]…[/AI]`, секция «Решения, принятые без владельца». |
